...
class YMNetworkTool: NSObject {
    ...
    /// 登陆
    func login(username: String,password:String , finished:@escaping (_ homeItems: [YMLogin]) -> ()) {
        ...
        Alamofire
            ...
            .responseJSON { (response) in
                ...
                if let value = response.result.value {
                    ...
                    //  字典转成模型
                    let item1 = data?[0].dictionary
                    let user = item1?["user"]?.dictionary
                    self.userId = (user?["id"]?.stringValue)!
                    self.token = (item1?["token"]?.stringValue)!
                    let homItems = [YMLogin]()
                        finished(homItems)
    ...
    /// 连接设备
    func connect(signId:String,finished:@escaping () -> ()) {
        //        let url = BASE_URL + "v1/channels/\(id)/items?gender=1&generation=1&limit=20&offset=0"
        let url = BASE_URL + "device/connect"
        let params = ["token": token,"signId":signId
    ...
    /// 断开连接
    func disconnect( finished:@escaping () -> ()) {
        //        let url = BASE_URL + "v1/channels/\(id)/items?gender=1&generation=1&limit=20&offset=0"
        let url = BASE_URL + "device/disconnect"
    ...
    /// 增加温度
    func add(finished:@escaping () -> ()) {
        //        let url = BASE_URL + "v1/channels/\(id)/items?gender=1&generation=1&limit=20&offset=0"
        let url = BASE_URL + "deivce/add"
        let params = ["token": token
        ]
        Alamofire
            .request(url, parameters: params)
    ...
    /// 获取首页顶部选择数据
    func loadHomeTopData(finished:@escaping (_ ym_channels: [YMMachine]) -> ()) {
//        https://kong4499.gicp.net:443/SWC_AC/device/findDevice
        let url = BASE_URL + "device/findDevice"
        print(url)
        let params = ["token": token,
                      "userID": userId]
        Alamofire
            ...
            .responseJSON { (response) in
                ...
                if let value = response.result.value {
                    ...
                    if let data = dict["data"].arrayObject {
                        var ym_machines = [YMMachine]()
                        for machine in data {
                            let ym_channel = YMMachine(dict: machine as! [String: AnyObject])
                            ym_machines.append(ym_channel)
                        }
                        finished(ym_machines)
                    }
                }
        }
    }
